...
export default function TableRow({ currentData }) {
  ...
  const handleRowClick = (id) => {
    ...
    navigate(`User?theTable=localReceived/${id}`);
  };
    ...
    <>
      {currentData?.map((item, i) => (
        ...
        >
          ...
          <td className="px-6 py-8 text-center">{item.local_financial_balance.toFixed(2)}</td>
          <td className="px-6 py-8 text-center">{item.international_financial_balance.toFixed(2)}</td>
          ...
          <td className="px-6 py-8 text-center">{item.total_card_number_for_reward}</td>
        </tr>
      ))}
    </>
  );
