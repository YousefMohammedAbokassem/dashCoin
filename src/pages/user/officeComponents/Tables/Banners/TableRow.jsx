...
export default function TableRow({ currentData, fetchData, setBody }) {
  ...
  const onDelete = async () => {
    ...
    try {
      await axios.delete(
        `${import.meta.env.VITE_API_URL}admin/banner/delete/${itemToDelete.id}`,
        {
          headers: {
  ...
  const handleSubmit = async () => {
    ...
    formDataToSend.append("banner_id", editData.id);
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_API_URL}admin/banner/update`,
        formDataToSend,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("access_token")}`,
